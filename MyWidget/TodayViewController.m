...
static NSString *const kGroupName = @"group.widgetMY";
...
- (void)viewDidLoad {
    ...
    // 数据处理(需要固化)
    NSUserDefaults *sharedUserDefaults = [[NSUserDefaults alloc] initWithSuiteName:kGroupName];
...
- (void)enterContainingApp {
    [self.extensionContext openURL:[NSURL URLWithString:@"medicalWdget://action=GotoHomePage"] completionHandler:^(BOOL success) {
